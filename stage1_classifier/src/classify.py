"""
classify.py — Module de classification d'offres d'emploi
=========================================================
Utilisé par:
  - Phase 2 (RAG): filtrer les offres avant indexation
  - Phase 3 (Agent): classifier les nouvelles offres quotidiennes

Usage standalone:
    python -m src.classify "Senior Data Engineer" "UBS" "Genève" "Pipelines Spark, dbt, Airflow"
...
# Permet l'import de ce module depuis n'importe quel répertoire de travail.
_BASE_DIR = Path(__file__).resolve().parent.parent

# Chemin vers le fichier d'état généré par src/03_finetuning.py.
# Ce fichier contient le model_id du modèle fine-tuné.
DEFAULT_STATE = _BASE_DIR / "results" / "training_logs" / "finetune_state.json"
    ...
    def _load_model_id(self) -> str:
        ...
        if not DEFAULT_STATE.exists():
            raise FileNotFoundError(
                ...
                "Passe model_id explicitement ou lance d'abord la Phase 4."
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m src.classify <title> [company] [location] [description]")
        sys.exit(1)
